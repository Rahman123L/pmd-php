...
<div x-data="{ isOpenn : true }">
    ...
    <main :class="isOpenn ? 'md:ml-60' : 'md:ml-20 ' + 'transition-all duration-500'" class="relative md:ml-60 pt-16 p-5 space-y-5">
        <div class="flex justify-between items-center">
            ...
            <h1 class="text-xs md:text-sm text-black font-medium"><span class="text-[#615EFC] font-medium">Home</span> / Ganti Password</h1>
        </div>
        ...
        <form x-data="{ passwordLama: '', passwordBaru: '', konfirmasi: '', passwordLamaBenar: 'Minimal8', alert: false, cekPassword: false, }" class="w-full sm:w-[50%] lg:w-[40%] bg-transparent space-y-5 md:space-y-6 rounded md:rounded-md">
            ...
            <div class="flex items-center gap-2 md:gap-3">
                ...
                <div x-data="{ modal: false }">
                    <button type="button" @click="(passwordLama === passwordLamaBenar) ? (passwordBaru === konfirmasi ? (modal = !modal, alert = false, cekPassword = false) : (alert = false, cekPassword = true)) : (alert = true, cekPassword = false)" :class="{ 'bg-[#615EFC] text-white cursor-pointer' : passwordLama && passwordBaru && konfirmasi, 'bg-[#979696] text-[#6A6A6A] cursor-not-allowed' : !(passwordLama && passwordBaru && konfirmasi) }" :disabled="!(passwordLama && passwordBaru && konfirmasi)" class="py-1.5 md:py-2 px-6 md:px-8 transition-colors duration-500 rounded shadow-md shadow-[#0000004D]">
